...
import custom_pandas_ta
from custom_pandas_ta import AnalysisIndicators


def bind(function_name, function, method):
    """
    Helper function to bind the function and class method defined in a custom
    indicator module to the active custom_pandas_ta instance.

    Args:
        function_name (str): The name of the indicator within custom_pandas_ta
        function (fcn): The indicator function
        method (fcn): The class method corresponding to the passed function
    """
    setattr(custom_pandas_ta, function_name, function)
...
def create_dir(path, create_categories=True, verbose=True):
    ...
    if create_categories:
        for sd in [*custom_pandas_ta.Category]:
...
def import_dir(path, verbose=True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid custom_pandas_ta categories
        if dirname not in [*custom_pandas_ta.Category]:
            if verbose:
                print(f"[i] Skipping the sub-directory '{dirname}' since it's not a valid custom_pandas_ta category.")
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            # figure out which of the modules functions to bind to custom_pandas_ta
            ...
            if module_name not in custom_pandas_ta.Category[dirname]:
                custom_pandas_ta.Category[dirname].append(module_name)
...
import_dir.__doc__ = \
"""
Import a directory of custom indicators into custom_pandas_ta
...
together with the existing custom_pandas_ta functions just like if they were part of
custom_pandas_ta.

If you at some late point would like to push them into the custom_pandas_ta library
...
indicators. Invoke custom_pandas_ta.custom.import_dir once to pre-populate it with
sub-folders for all available indicator categories, e.g.:

>>> import os
>>> from os.path import abspath, join, expanduser
>>> from custom_pandas_ta.custom import create_dir, import_dir
>>> ta_dir = abspath(join(expanduser("~"), "my_indicators"))
>>> create_dir(ta_dir)

3. You can now create your own custom indicator e.g. by copying existing
ones from custom_pandas_ta core module and modifying them.
...
available in categories under the custom_pandas_ta-folder. The only difference will
...
like all other native indicators in custom_pandas_ta, including help functions.
"""
